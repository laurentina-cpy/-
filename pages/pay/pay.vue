...
const handlePay = async () => {
  ...
  try {
    ...
    // 保存payment_id用于后续回调
    orderInfo.value.paymentId = payParams.payment_id
    ...
    if (selectedMethod.value === 'wechat') {
      ...
      uni.showToast({ title: '暂不支持此支付方式', icon: 'none' })
    }
  } catch (error) {
    uni.hideLoading()
...
const handlePaySuccess = async () => {
  try {
    // 调用后端支付回调接口，更新订单状态
    const paymentId = orderInfo.value.paymentId
    console.log('回调支付ID:', paymentId)

    const res = await post('/pay/callback', {
