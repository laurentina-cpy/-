    ...
    <scroll-view class="order-scroll" scroll-y>
      ...
      <view class="order-item" v-for="(order, index) in filteredOrderList" :key="order.order_id">
        ...
        <view class="order-goods">
          <view class="goods-item" v-for="goods in (order.orderItems || [])" :key="goods.order_item_id || goods.goods_id">
        ...
        <view class="order-footer">
          ...
          <view class="btn-group">
            ...
            <button class="operate-btn primary" v-if="order.status === 0" @click="payOrder(order)">去支付</button>
            <button class="operate-btn" v-if="order.status === 1 || order.status === 2" @click="confirmReceipt(order.order_id)">确认收货</button>
            <button class="operate-btn" v-if="order.status === 4 || order.status === 5" @click="deleteOrder(order.order_id)">删除订单</button>
          </view>
        </view>
      </view>
...
const payOrder = (order) => {
  ...
  // 跳转到支付页面
  const orderParams = encodeURIComponent(JSON.stringify(orderInfo));
  uni.navigateTo({
    url: `/pages/pay/pay?order=${orderParams}`
  });
};
...
const confirmReceipt = async (orderId) => {
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        try {
          const result = await confirmReceiptAPI(orderId);
          if (result.code === 200) {
