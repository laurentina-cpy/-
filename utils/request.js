...
// 后端接口基础地址
// 开发工具使用localhost，真机使用局域网IP或公网地址
// 如果连接失败，请确认：
// 1. 电脑和手机在同一WiFi
// 2. 后端服务器正在运行（npm start）
// 3. 防火墙允许3000端口
//
// 你的电脑有以下IP地址，请根据手机连接的WiFi选择：
// - 192.168.192.1
// - 192.168.152.1
// - 10.67.178.85 (当前使用)
// - 172.20.208.1
    ...
    uni.request({
      ...
      fail: (err) => {
        ...
        if (isDev) {
          ...
          // 真机连接本地服务器提示
          if (isRealMachine() && err.errMsg && err.errMsg.includes('CONNECTION_REFUSED')) {
            uni.showModal({
              title: '连接失败',
              content: '真机无法连接localhost，请修改utils/request.js中的BASE_URL为局域网IP或公网地址',
              showCancel: false
            });
            return;
          }
