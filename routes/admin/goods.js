...
router.get('/list', checkPermission('goods:view'), async (req, res) => {
  try {
    ...
    if (supplier_id && supplier_id !== '') where.supplier_id = supplier_id;
    if (shelf_status !== undefined && shelf_status !== '') where.shelf_status = shelf_status;
    if (keyword && keyword !== '') {
...
// 批量删除
router.delete('/batch', checkPermission('goods:delete'), async (req, res) => {
  try {
    const { goods_ids } = req.body;

    if (!Array.isArray(goods_ids) || goods_ids.length === 0) {
      return res.json({ code: 400, message: '请选择要删除的商品' });
    }
