...
// 确认收货
router.patch('/confirm/:order_id', async (req, res) => {
  try {
    // 从 Authorization 头解析 Token
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
    ...
    // 验证订单状态（配送中或已送达可以确认收货）
    if (order.status !== 2 && order.status !== 3 && order.status !== 1) {
      return res.json({ code: 400, message: '订单状态不正确，无法确认收货' });
    }

    // 更新订单状态为已完成
    await order.update({ status: 4, finish_time: new Date() });

    res.json({ code: 200, message: '订单已完成' });
  } catch (err) {
    console.error('确认收货失败:', err);
    res.status(500).json({ code: 500, message: '确认收货失败：' + err.message });
  }
});
